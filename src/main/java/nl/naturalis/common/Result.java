...
import static nl.naturalis.check.CommonExceptions.noSuchElement;
...
public final class Result<T> implements Emptyable {
   ...
   */
  public T get() {
    Check.that(isAvailable()).is(yes(), noSuchElement("no result available"));
    return val;
  }
  ...
  /**
   * Returns {@code true} if no result was available or the result is empty as per
   * {@link ObjectMethods#isEmpty(Object)}.
   *
   * @return {@code true} if no result was available or the result is empty as per
   *     {@link ObjectMethods#isEmpty(Object)}
   */
  @Override
  public boolean isEmpty() {
    return this == NONE || ObjectMethods.isEmpty(val);
  }

  /**
   * Returns {@code true} if a result was available and it was deep-not-empty as per
   * {@link ObjectMethods#isDeepNotEmpty(Object)}
   *
   * @return {@code true} if a result was available and it was deep-not-empty as per
   *     {@link ObjectMethods#isDeepNotEmpty(Object)}
   */
  @Override
  public boolean isDeepNotEmpty() {
    return this != NONE && ObjectMethods.isDeepNotEmpty(val);
  }
